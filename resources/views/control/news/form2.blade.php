...
<div class="tab-content" id="myTabContent">
    ...
    <div class="tab-pane fade {{ $loop->index == 0 ? 'show active' : '' }}" id="{{ $key }}" role="tabpanel" aria-labelledby="{{ $key }}-tab">
        ...
        <div class="form-group">
            ...
            <input type="text" id="{{ $key }}" name="title[{{ $key }}]" value="{{ old('title.'. $key, $news->getTranslation('title', $key)) ?? $news->title }}" class="form-control">
            @error('title.'.$key)
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="{{ $key }}" rows="3" name="summary[{{ $key }}]">{!! old('summary.'. $key, $news->getTranslation('summary', $key))??$news->summary !!}}</textarea>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="{{ $key }}" rows="20" name="details[{{ $key }}]">{!! old('details.'. $key, $news->getTranslation('details', $key))??$news->details !!}}}</textarea>
            @error('details.'.$key)
            <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
    ...
    <div class="form-group">
        ...
        <select name="classification_id" class="form-control">
            ...
            <option value="{{ $class->id }}" {{ $class->id == $news->classification_id ? 'selected' : '' }}>{{ $class->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="mb-3">
        ...
        @if ($news->image)
        <img alt="" src="{{asset('/storage/photos/web/'.$news->image)}}" width="200px" />
        @endif
    ...
    <div class="form-group">
        ...
        <input type="text" name="video" value="{{ old('video') ?? $news->video }}" class="form-control">
        @error('video')
        <small class="text-danger">{{ $message }}</small>
        @enderror
    </div>
