        ...
        <div class="tab-pane fade {{ $loop->index == 0 ? 'show active' : '' }}" id="{{ $key }}" role="tabpanel"
            ...
            <div class="form-group">
                ...
                <input type="text" id="{{ $key }}" name="title[{{ $key }}]"
                       value="{{ old('title.'. $key) ?? $news->title }}" class="form-control">
                @error('title.'.$key)
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="{{ $key }}" rows="3"
                          name="summary[{{ $key }}]">{!!  old('summary.'. $key)??$news->summary!!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="{{ $key }}" rows="20"
                          name="details[{{ $key }}]">{!!old('details.'. $key)??$news->details  !!}</textarea>
                @error('details.'.$key)
                <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
